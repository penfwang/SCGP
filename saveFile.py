...
def save_individual(randomSeeds, dataSetName,individuals):
    with open(str(randomSeeds) + 'one' + dataSetName+'.pickle', 'wb') as file:
        pickle.dump(individuals, file, protocol=pickle.HIGHEST_PROTOCOL)
    file.close()
    return

def save_archive(randomSeeds, dataSetName,individuals):
    with open(str(randomSeeds) + 'archive' + dataSetName+'.pickle', 'wb') as file:
        pickle.dump(individuals, file, protocol=pickle.HIGHEST_PROTOCOL)
    file.close()
    return

def save_pop(randomSeeds, dataSetName,individuals):
    with open(str(randomSeeds) + 'pop' + dataSetName+'.pickle', 'wb') as file:
        pickle.dump(individuals, file, protocol=pickle.HIGHEST_PROTOCOL)
    file.close()
    return
